...
describe("carts works fine when", () => {
  ...
  const increaseQuantity = jest.fn().mockReturnValue(
    {
      id: 1,
      title: "Book 1",
      imgUrl: `${legacyCoder}`,
  ...
  it("displays correct symbol of increment button", () => {
    mockCartItems.forEach((book) => {
      const increaseQuantityButton = screen.getByTestId(
        `incrementQuantity${book.id}`
      );
      expect(increaseQuantityButton.textContent).toBe(
        TestConstants.INCREMENT_QUANTITY_BUTTON_SYMBOL
      );
    });
  });
  ...
  it("displays correct symbol of decrement button", () => {
    mockCartItems.forEach((book) => {
      const decrementQuantityButton = screen.getByTestId(
        `decrementQuantity${book.id}`
      );
      expect(decrementQuantityButton.textContent).toBe(
        TestConstants.DECREMENT_QUANTITY_BUTTON_SYMBOL
      );
    });
  });
